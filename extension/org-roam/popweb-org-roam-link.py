...
def pop_org_roam_link_window(popweb, module_path, module_name, index_file, index_path,
                             x, y, x_offset, y_offset,
                             frame_x, frame_y, frame_w, frame_h,
                             width_scale, height_scale, show_window,
                             script_file, media_directory,
                             new_html_p, html_string, translation,
                             eval_in_emacs_func):
    ...
    if translation:
        html_string = f'<div id="org-roam-link-preview" class="left" style="height: 100%; width: 50%; position: fixed; z-index: 1; top: 0; overflow-x: hidden; padding-top: 20px">{html_string}</div><div id="translation" class="right" style="height: 100%; width: 50%; position: fixed; z-index: 1; top: 0; overflow-x: hidden; padding-top: 20px">{translation}</div>'
    else:
        html_string = f'<div id="org-roam-link-preview" class="left" style="height: 100%; width: 100%; position: fixed; z-index: 1; top: 0; overflow-x: hidden; padding-top: 20px">{html_string}</div>'
    index_html = open(index_file, "r").read().replace(
        "HTML-PLACEHOLD", html_string).replace(
            "HEAD-PLACEHOLD", get_tooltip_script(script_file))
    web_window.webview.setHtml(index_html, QUrl(index_path))
